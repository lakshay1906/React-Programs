...
const Card = ({ data }) => {
  ...
  return (
    !showScore && (
      <counterContext.Provider value={{ counter, btnDisabled, setBtnDisabled }}>
        <CorrectAnswer.Provider value={data.results[counter].correct_answer}>
          <div
            id="container"
            className="w-10/12 h-4/5 p-5 text-invert backdrop-blur-xl rounded-3xl flex flex-col items-center gap-8 border border-black "
          >
            ...
            <div
              id="options"
              className="border border-red-700 w-10/12 flex flex-col flex-wrap items-center justify-center h-2/4 bg-white rounded-lg gap-y-10 px-10"
            >
